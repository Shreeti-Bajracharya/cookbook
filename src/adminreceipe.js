...
const Addreceipe = styled.section`
.main-page{
    position:fixed;
    margin-top:82px;
    margin-left:50px;
...
`;
export default function AdminAddReceipe() {
    const  {getpost,receipee,setreceipee,receipeeRef,deletedata} = useProductContext()
    ...
    const receipeRef= collection(db,"receipe")
    ...
    const add=async(e)=>{
        e.preventDefault()
          try{
                               await addDoc(receipeRef,{
                            "title":title,"desc":desc,"head":head,"category":category,"imageUrl":cover,"slug":slug
                                    })
            navigate("/addproduct")
       }
        catch(e){
        ...
        <Addreceipe>
            <div>
                ...
                <div id="activity">
                    ...
                    <div id="d2">
                      <form onSubmit={add}  action='/admin/home/product/add' >
                        <div className='div4'>
                            <label>Title</label>
                            <input type='text' style={{width:"400px",height:"30px"}} value={title} onChange={(e)=>{settitle(e.target.value)}} />
                        </div>
                        <div className='div4'>
